...
async function creatingRoles(chatID, counter) {
    ...
    if (counter <5) {
        masRoles = ['Cin', 'Həkim', 'Uğurlu']; //2
        counterWorld = 2;
        counterMafia = 1;
    } else if (counter <7) {
        masRoles = ['Cin', 'Həkim', 'Komissar', 'Uğurlu'];//2
        counterWorld = 3;
    } else if (counter <9) {
        masRoles = ['Cin', 'Ruh', 'Həkim', 'Komissar', 'Uğurlu', 'Kamikadze'];//3
        counterWorld = 4;
    } else if (counter <10) {
        masRoles = [
            'Cin', 'Ruh', 'Həkim', 'Komissar', 'Uğurlu', 'Kamikadze', 
            'Mühafizəçi', 'Manyak'
        ];//2
        counterWorld = 6;
    } else if (counter <11) {
        ...
        masRoles = [
            'Cin', 'Ruh', 'Həkim', 'Komissar', 'Leytenant', 'Uğurlu', 
            'Kamikadze', 'Mühafizəçi', 'Manyak', 'Iblis', 'Məşuqə'
        ];//3
        counterWorld = 8;
    } else if (counter <15) {
        masRoles = [
            'Cin', 'Ruh', 'Həkim', 'Komissar', 'Leytenant', 'Uğurlu', 
            'Kamikadze', 'Mühafizəçi', 'Manyak', 'Iblis', 'Məşuqə', 'Zombi'
        ];//4
        counterWorld = 8;
        counterTriada = 1;
    } else if (counter <19) {
        masRoles = [
            'Cin', 'Ruh', 'Həkim', 'Komissar', 'Leytenant', 'Uğurlu', 
            'Kamikadze', 'Mühafizəçi', 'Manyak', 'Iblis', 'Məşuqə', 'Zombi', 'Dəli'
        ];//7
        counterWorld = 8;
        counterTriada = 2;
    }
...
function distributionOfRoles(ChatID, masRoles, masPlayers) {
    masPlayers.forEach((item, i) => {
        ...
        if (masRoles[i] == 'Komissar' || masRoles[i] == 'Leytenant') {
            ...
            allies = 3;
        } else if (masRoles[i] == 'Manyak' || masRoles[i] == 'Iblis') {
            allies = 4;
        }
...
function createTextMessageRoles(role) {
    ...
    switch(role) {
        ...
        case 'Manyak':
            ...
            break;
        case 'Iblis':
                textMessage = 'Rolun - 🔪 <b>Iblis</b>.\Manyak ile birge mafiyaya qarsi mübarizə aparmaq istəyir, istənilən insanı öldürə bilər...';
            break;
        case 'Məşuqə':
...
async function sendDayMessageLivePlayers(ChatID, data) {
    ...
    if (masRole[13]==1) { listRoles+=`🔪 Iblis, `; }
    await app.bot.telegram.sendMessage(
        ChatID,
